...
- (void)startStreamingFromFileOrMagnetLink:(NSString *)filePathOrMagnetLink
    ...
    if ([filePathOrMagnetLink hasPrefix:@"magnet"]) {
        ...
        if ([[NSFileManager defaultManager] fileExistsAtPath:filePath]) {
            ...
            error = [[NSError alloc] initWithDomain:@"com.popcorntimetv.popcorntorrent.error" code:-2 userInfo:@{NSLocalizedDescriptionKey: [NSString localizedStringWithFormat:@"File doesn't exist at path: %@", filePath]}];
        }
    ...
    if (!basePath) {
        NSError *error = [NSError errorWithDomain:@"com.popcorntimetv.popcorntorrent.error" code:-412 userInfo:@{NSLocalizedDescriptionKey: @"Could not create download directory"}];
        if (failure) failure(error);
        return [self cancelStreamingAndDeleteData:NO];
    }
...
- (void)processTorrent:(torrent_handle)th {
    ...
    if (self.readyToPlayBlock) {
        ...
        NSLog(@"file to be streamed is %@", fileURL);
        __weak __typeof__(self) weakSelf = self;
        dispatch_async(dispatch_get_main_queue(), ^{
            if (weakSelf.readyToPlayBlock) weakSelf.readyToPlayBlock(fileURL, fileURL);
        });
    }
}
...
- (void)metadataReceivedAlert:(torrent_handle)th {
    ...
    if (_requiredSpace > availableSpace.longLongValue) {
        NSString *description = [NSString localizedStringWithFormat:@"There is not enough space to download the torrent. Please clear at least %@ and try again.", self.fileSize.stringValue];
